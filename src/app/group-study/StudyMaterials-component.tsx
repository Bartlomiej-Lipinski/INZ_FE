...
import FileFilters from "@/app/group-study/StudyMaterialsFilters-component";
import CategoriesManagerComponent from '../../components/pages/CategoriesManager-component';
...
export function StudyMaterialsPage({
    ...
    const handleUploadFile = async () => {
        ...
        try {
            ...
            if (!groupData?.id) {
                showError('Brak danych grupy');
                return;
            }

            if (!userId) {
                showError('Nie znaleziono danych użytkownika. Zaloguj się ponownie');
                return;
            }
            ...
            showError('Nie udało się przesłać pliku. Spróbuj ponownie.');
        }
    };
    ...
    const handleDownload = async (file: StoredFileResponseDto) => {
        try {
            ...
            showError('Nie udało się pobrać pliku. Spróbuj ponownie.');
        }
    };
    ...
    const handleDeleteFile = async () => {
        ...
        if (!fileToDelete) {
            showError('Nie znaleziono pliku');
            handleCloseMenu();
            return;
        }
        if (!isAdmin && fileToDelete.uploadedById !== userId) {
            showError('Nie masz uprawnień do usunięcia tego pliku');
            handleCloseMenu();
            return;
        }
        ...
        try {
            ...
            handleCloseMenu();
            showSuccess('Plik został usunięty pomyślnie.');
        } catch (error) {
            console.error('Błąd podczas usuwania pliku:', error);
            showError('Nie udało się usunąć pliku. Spróbuj ponownie.');
            handleCloseMenu();
        }
    };
    ...
    const handleAddCategory = async () => {
        ...
        try {
            ...
            showError('Nie udało się dodać kategorii. Spróbuj ponownie.');
        }
    };
    ...
    const handleDeleteCategory = () => {
        ...
        if (hasFiles) {
            showError('Nie można usunąć kategorii, która zawiera pliki. Najpierw usuń lub przenieś pliki.');
            handleCloseCategoryMenu();
            return;
        }
                    ...
                    >
                        ...
                        <DialogActions sx={{px: 3, pb: 2}}>
                            <Button onClick={() => setUploadDialogOpen(false)}
                                    sx={{bgcolor: 'error.main'}}>Anuluj</Button>
                            <Button
                                variant="contained"
