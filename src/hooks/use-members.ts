...
export function useMembers(): UseMembersResult {
    ...
    const grantAdminPrivileges = useCallback(async (groupId: string, userId: string): Promise<ApiResponse<unknown>> => {
        ...
        try {
            ...
            if (message === "User already has admin privileges.") {
                setError("Użytkownik posiada już uprawnienia administratora.");
                return { success: false, message: "Użytkownik już ma uprawnienia administratora." };
            } else {
                setError(message);
                return { success: false, message };
            }
